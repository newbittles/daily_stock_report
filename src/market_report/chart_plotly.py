...
def render_interactive(candles, ticker: str, name: str, date: str | None = None,
    ...
    fig.update_layout(
        ...
        height=950,
        # rangeslider 제거 (박스 줌 방해). 축별 독립 줌(트레이딩뷰식) 활성화:
        #   본문 드래그=박스줌 / x축(시간)위 드래그·휠=가로 세분화 / y축(가격)위 드래그·휠=세로 세분화
        xaxis=dict(domain=[0, 1], anchor="y4", rangeslider=dict(visible=False), fixedrange=False),
        yaxis=dict(domain=[0.46, 1.0], title="가격", fixedrange=False, autorange=True),
        yaxis2=dict(domain=[0.31, 0.44], title="거래량", fixedrange=False),
        yaxis3=dict(domain=[0.16, 0.29], title="MACD", fixedrange=False),
        yaxis4=dict(domain=[0.0, 0.14], title="RSI", range=[0, 100], fixedrange=False),
