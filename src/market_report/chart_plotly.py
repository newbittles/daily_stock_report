...
def render_interactive(candles, ticker: str, name: str, date: str | None = None,
    ...
    fig.update_layout(
        ...
        height=950,
        # rangeslider 제거 (드래그 박스 줌 방해). 드래그=영역확대가 정상 작동.
        xaxis=dict(domain=[0, 1], anchor="y4", rangeslider=dict(visible=False)),
        yaxis=dict(domain=[0.46, 1.0], title="가격"),
        yaxis2=dict(domain=[0.31, 0.44], title="거래량"),
        yaxis3=dict(domain=[0.16, 0.29], title="MACD"),
        yaxis4=dict(domain=[0.0, 0.14], title="RSI", range=[0, 100]),
