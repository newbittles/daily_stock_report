...
- **NXT 시간외 조사**(미구현, 사용자 결정 대기): 마감후 리포트에 '정규장 마감후 시간외 상위상승률' 추가 요청. 실측: KIS `/ranking/fluctuation`에 `fid_cond_mrkt_div_code=NX`(넥스트레이드) 넣으면 NXT 데이터 정상수신(UN=통합은 ERR). ⚠️ 단 prdy_ctrt는 전일종가대비(정규장 포함)라 '마감후 only'와 다름 → 정확한 '정규장종가 대비 시간외변동'은 **시간외 세션(15:30~20:00)에 실데이터로 필드/계산 검증 필요**(오전엔 오늘 종가 없어 검증불가). 사용자에 "오늘 오후 시간외에 검증·구현" 제안함(응답 대기).
...
- **origin/main 최신 커밋**: `652cd87` (과열 추천 강등 수정). 2026-06-05 세션 작업 전부 푸시·배포 완료. (NXT 시간외만 미구현=사용자 결정 대기, 0c 참조)
