...
- **✅ P1 완료 — 자동매매 라이브 전환(06-08 00시대, 사용자 "지금 등록")**: dry-run 게이트 생략(모의계좌=실위험0 + 에러알림 구비 근거). 서버 crontab 등록: **평일 15:10 buy --send / 15:15 sell --send**(둘 다 연속거래장 내 — 사용자가 동시호가 체결 불확실성 지적해 15:20→15:10 변경. 15:50 sell 계획 폐기=장마감후 거부 위험). 1회성 dry-run 크론은 제거(중복 방지). 로그 `logs/auto_trader.log`. pre 리포트(14:50 시작)는 3~4분에 top3 JSON 완료(06-05 실측) → 15:10까지 여유. **오늘(06-08) 15:10 첫 라이브 매수 — 체결·odno 텔레그램 확인 필요.**
- **코인 신호 백테스트(06-08, 사용자 요청)**: XRP 25/8·26/5·25/6·25/7 + ETH 25/4·25/7 재현(일봉, 미래누출X). 결론: A=랠리초입 2/3 적중(XRP 7/9 +48%·ETH 7/2 +48%·XRP 8/7 -5%컷), 🔻하락전환 경고 2/2, 바닥다지기 구간 오탐 0, 과열라벨≠매도신호 재확인. 텔레그램 보고 발송됨.
- **E 전수 백테스트(06-08, 4H게이트 완전재현)**: 5코인×800일에서 **실제 E 발동 단 2회**(SOL·DOGE 25/2/25 — +5일 +28/+17%, +20일 마이너스=데드캣). 일봉부 44회 중 42회를 4H 게이트가 거름(탈락군 +20일 -7.9% 칼날 — 게이트 가치 입증). ⚠️정정: 'ETH 4/9 E 대바닥 +115%'는 일봉부 기준, 실제 E(4H RSI 38)는 미발동. **결론: 코인 E=드문 단기반등 신호. 파라미터 보정 안 함**(승자·패자 같은 RSI대역, 조정으로 분리 불가 + 주식 공유 config 충돌 회피).
- **보류 3건 종결(06-08, 사용자 "충돌나면 넘겨")**: ①프리장 TOP5 텔레그램 반영(edbc721, us_premarket 한정 — overview-only 정책의 승인된 예외) ②**서버 screener.yaml = 축소판 유지 확정**(origin 통일은 RAM 1GB 충돌 → 반영 안 함, 보류 종결) ③코인 전략 파라미터 보정 안 함.
...
- **분석(e5612c6 최종)**: 유니버스 **6개 고정**(USDT 최상단·BTC·ETH·XRP·SOL·DOGE, 사용자 축소). **텔레그램=시세줄 + 'ㄴ일봉 🔵바닥권 · 4시간봉 🔻하락전환 · 전략 없음/B·E' 한 줄만**(전략 미매칭도 '없음' 명시 — 누락 오인 방지). **웹=모바일 카드**(테이블 폐기) + 상세(일봉/4H 각각 신호등(coin_phase, 코인 과열임계 120≥30/60≥20)·20/60이격·RSI·MACD라벨·ABCDE). E=일봉 부착(4H RSI≤30 게이트)+코인F&G≤25=🔥시장동반바닥. USDT analyze=False(스테이블 A/C/D 오탐).
