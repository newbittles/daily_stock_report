...
## 12. 백테스트 고도화 결과 (2026-06-03) — P3 1차 결론 정정

P3 §11 한계를 개선: 유니버스 nasdaq급등주→**S&P500 대형주**(급등 편향↓), 기간 300→**750일**(하락 포함),
청산 MA단순→**전략별 정확청산**(A 복합: 구름/MACD/20선, `make_a_exit`), **거래비용 왕복 0.1% 차감**.
엔진: `backtest_with_exit`+`make_*_exit` (테스트 9개).

**결과(S&P500 120종목 × 750일, 비용 0.1%):**

| 전략 | 진입 | 승률 | 평균 | 중앙 | 최악 | 보유 |
|------|------|------|------|------|------|------|
| A | 2763 | 33.8% | +0.70% | -1.23% | -36.9% | 12일 |
| B | 339 | 41.9% | +3.70% | -1.11% | **-23.1%** | 16일 |
| C | 1842 | 40.0% | +4.09% | -2.25% | -38.7% | 45일 |
| D | 1990 | 35.4% | +3.83% | -2.28% | -34.7% | 39일 |

**결론(중요 — 1차 정정):**
- **1차 C 평균 +79%는 강세장 편중 + 생존편향(거래대금 급등주)의 환상.** 고도화 후 +4%로 현실화.
- **모든 전략 중앙값 음수** = 절반 이상 손실, 평균은 소수 대박 의존. 기계적 매매 엣지 약함.
- → **"참고용 시그널 + 사용자 최종판단"이라는 설계 철학이 데이터로 확인됨**(CLAUDE.md §2 면책). 자동매매 전략 아님.
- B가 상대적 우위(최악 -23%, 승률 41.9% 최고). A는 과다진입(2763). D는 lookback 여전 무효.

**잔존 한계:** S&P500 '현재 구성'(과거 탈락종목 미포함 생존편향 잔존), 750일≈2023~2026(2022 약세장 일부 미포함). 절대 결론보다 **전략 간 상대 비교 + 시그널 보조** 용도로 해석.

## 13. P4 리포트 통합 (2026-06-03 완료)

**충돌 회피:** `market_report` 코어(models/pipeline/analyzer)는 UI 담당 작업 영역 → 미수정.
us_screening 전용 **독립 모듈** `src/screener/us_report.py`로 분리, 텔레그램 발송 경로
(`settings.allowed_chat_ids` + `Bot` + `TelegramNotifier`)만 재사용.

- `build_us_screening_report(picks, top_n)` — **순수**(값→Markdown), 테스트 가능
- `send_us_screening_report(picks)` — 외부(텔레그램 화이트리스트 발송)
- 형식: **전략별 Top N**(백테스트 우위순 C·B·A·D), 거래대금 상위, 섹터·근거수치 + **면책 + 백테스트 보조 주의**
- `scripts/run_us_screening.py --send` — 기본 미리보기, `--send`로만 실제 발송(외부 행동은 명시적 승인)
- 검증: combined 54종목 → 전략별 Top4 미리보기 정상 렌더. 테스트 2개(build 순수). 실제 발송은 사용자 승인 영역.

**P1~P4 + 고도화 전체 완료.** engine·indicators·patterns·한국 pipeline·market_report 코어 **수정 0** 유지(충돌 회피).

## 14. cross_signal 보조신호 (2026-06-03)

대세상승주 매매 시 **단기조정/고점 판단 보조신호**. 5일선<10일선(단기 데드크로스) 상태에서 20일 이격으로 구분:
- 이격 **≥15% → 🟢 단기눌림**(아직 추세 위, 매수 기회)
- 이격 **≤7% → ⚠️ 조정시작**(20선 근접, 경고)

**domain SSOT:** `patterns.core.cross_signal()` 순수함수 신규(한국 `strategy_section._cross`와 동일 기준).
미국 반영: `USStockPick.cross_signal` 산출 + `us_report` 배지 표기. 한국 인라인 `_cross`는 동일 기준으로
이미 존재 → **추후 이 함수로 통일**(현재 market_report=UI 에이전트 영역이라 충돌 회피로 보류).
검증: combined 54종목 중 CVX correction 표기. 테스트 5개(pullback/correction/none/배지).
